...
{
    v[0][0]=0;
    if(i==0) v[i][j]= v[i][j-1] +score('_',B[j]);
    if(j==0) v[i][j]= v[i-1][j] +score(A[i],'_');
    int op1 = v[i-1][j-1]+score(A[i],B[j]);
    int op2 = v[i-1][j]+score(A[i],'_');
    int op3 = v[i][j-1]+score('_',B[j]);
    v[i][j] =max(op1,max(op2,op3));
}
*/
int main()
{
    v[0][0] = 0;
    repl(j,B.length()) v[0][j] = v[0][j-1] + score('_',B[j-1]);
    repl(i,A.length()) v[i][0] = v[i-1][0] + score(A[i-1],'_');
    repl(i,A.length())
    {
        repl(j,B.length())
        {
            /*if(i==0 or j==0){ printf("%2d ",v[i][j]); continue;}
            if(i==0) {v[i][j]= v[i][j-1] +score('_',B[j]); goto A;}
            if(j==0) {v[i][j]= v[i-1][j] +score(A[i],'_'); goto A;}*/
            int op1 = v[i-1][j-1]+score(A[i-1],B[j-1]);
            int op2 = v[i-1][j]+score(A[i-1],'_');
            int op3 = v[i][j-1]+score('_',B[j-1]);
            v[i][j] =max(op1,max(op2,op3));
            //printf("%2d ",v[i][j]);
        }
        //puts("");
